#let periodic_table = {
  ...
  let element(number, mass, symbol, name, fill: white, text-color: black) = {
    box(width: 3cm, height: 3cm, fill: fill, stroke: black, inset: 4pt)[
      #set align(center)
      #text(size: 18pt, weight: "bold")[#number #h(1fr) #mass]#linebreak()
      #v(1fr)
      #text(size: 40pt, weight: "bold", fill: text-color)[#symbol]#linebreak()
      #v(1fr)
      #text(size: 13pt)[#name]
    ]
  }
